...
class QunuanQiSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        max_page = driver.find_element_by_xpath(
            ".//div[@id='filter-results']/div[@id='bottom_pager']/span[@class='page-more']").text
        results = driver.find_element_by_xpath(".//span[@class='total-result']/strong[@id='totalNum']").text
        ...
        for i in range(1, int(max_number) + 1):
            print('第%d页' % i)
            element = driver.find_elements_by_xpath(".//div[@class='res-info']/p[@class='sell-point']/a")
            for each in element:
            ...
            try:
                ...
                try:
                    driver.refresh()
                    time.sleep(5)
                    driver.find_element_by_xpath(".//span[@class='page-more']/input[@id='bottomPage']").send_keys(
                        '%d' % i)
                    driver.find_element_by_xpath(".//div[@id='bottom_pager']/a[@class='page-more ensure']").send_keys(
                        Keys.ENTER)
                except:
                    print('失败！')
                    break
        ...
        dataframe.to_csv('苏宁取暖器1211.csv', index=None)
        # driver.quit()
